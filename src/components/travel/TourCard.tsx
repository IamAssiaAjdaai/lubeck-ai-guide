...
import { Clock3, Footprints, Route, ChevronRight } from "lucide-react";
...
export default function TourCard({
  ...
  return (
    <section className="tour-suggestion surface-card overflow-hidden p-3">
      <div className="flex items-start gap-3">
        {image && (
          <div className="tour-suggestion-photo relative shrink-0 overflow-hidden rounded-xl">
            <Image
              src={image}
              alt=""
              fill
        ...
        <span
          className={`${image ? "hidden" : "flex"} h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white text-accent shadow-sm`}
        >
          <Footprints aria-hidden="true" size={21} strokeWidth={1.8} />
        </span>
        <div className="min-w-0">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-text-secondary">
            ...
            <p className="flex items-center gap-1.5">
              <Route aria-hidden="true" size={16} strokeWidth={1.8} />
              {stops}
            </p>
          </div>
        </div>
          ...
          <ol className="mt-3 grid gap-2">
            {stopNames.map((stop, index) => (
              ...
              >
                <span
                  aria-hidden="true"
                  className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white text-xs font-semibold text-blue-700"
                >
                  {index + 1}
                </span>
                <span
      ...
      <TrackedLink
        ...
        properties={{
          locale,
          start_landmark_slug: startLandmarkSlug,
          tour_id: tourId,
        }}
        className="tour-suggestion-link"
      >
        {ctaLabel}
