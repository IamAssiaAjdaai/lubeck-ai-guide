...
export type TourPreferenceRankingOptions = Readonly<{
  origin?: GeographicCoordinates;
}>;

export function isTourInterest(value: unknown): value is TourInterest {
  return TOUR_INTERESTS.some((interest) => interest === value);
}

export function isWalkingPreference(value: unknown): value is WalkingPreference {
  return WALKING_PREFERENCES.some((preference) => preference === value);
}

export function parseTourPreferences(value: unknown): TourPreferences {
  const rawInterests =
    typeof value === "object" && value !== null && "interests" in value
      ? value.interests
      : undefined;
  if (
    typeof value !== "object" ||
    ...
    !isWalkingPreference(value.walkingPreference)
  ) {
    return DEFAULT_TOUR_PREFERENCES;
  }
  return {
    interests: TOUR_INTERESTS.filter((interest) => rawInterests.includes(interest)),
    walkingPreference: value.walkingPreference,
  };
}
...
export function countTourInterestMatches(
  ...
  return interests.reduce(
    (matches, interest) =>
      matches + Number(place.tags?.includes(TOUR_INTEREST_TAGS[interest]) ?? false),
    0,
  );
}
...
>(
  ...
  const useDistance =
    preferences.walkingPreference === "less-walking" && Boolean(options.origin);
  return places
    .map((place, originalIndex) => ({
      place,
      originalIndex,
      matches: countTourInterestMatches(place, preferences.interests),
      distance: useDistance && options.origin
        ? calculateDistanceMeters(options.origin, place.coordinates)
        : undefined,
    }))
    .sort((first, second) => {
      const matchDifference = second.matches - first.matches;
