...
/** Geographic straight-line distance, not pedestrian routing distance. */
export function calculateDistanceMeters(
  ...
  const centralAngle =
    2 * Math.atan2(Math.sqrt(normalized), Math.sqrt(1 - normalized));
...
export function calculatePlaceDistance(
  ...
  const walkingMinutes = estimateWalkingMinutes(distanceMeters, walkingSpeedKmh);
  return walkingMinutes === undefined ? undefined : { distanceMeters, walkingMinutes };
}
...
>(
  ...
  return places.map((place) => withPlaceDistance(place, origin, walkingSpeedKmh));
}

export function formatDistance(distanceMeters: number, locale: string): string | undefined {
...
export function localizePlaceDistance(
  ...
  return distanceLabel && walkingTimeLabel
    ? { ...distance, distanceLabel, walkingTimeLabel }
    : undefined;
}
...
>(places: readonly T[]): readonly T[] {
  return [...places].sort((first, second) => {
    ...
    if (firstDistance === undefined && secondDistance === undefined) return 0;
