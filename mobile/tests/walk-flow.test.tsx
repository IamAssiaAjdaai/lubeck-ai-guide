...
describe("rendered native V2 flow (native bridges mocked, not device acceptance)", () => {
  ...
  it("opens a place-detail add request as a proposal, without silently changing the route", async () => {
    const time = Date.now();
    const active = { id: "active", citySlug: "city", settings: { minutes: 120, interests: ["history"], walking: "balanced", start: places[0].coordinates }, remaining: ["place-0"], visited: [], position: places[0].coordinates, historyDistance: 0, startedAt: time };
    mocks.load.mockResolvedValueOnce(active);
    render(<NativeWalkFlow citySlug="city" cityName="City" places={places} addSlug="place-1" />);
    await screen.findByRole("dialog");
    expect(mocks.persist).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole("button", { name: walkCopy("en").confirm }));
